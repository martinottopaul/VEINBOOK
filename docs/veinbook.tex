...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{df <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{a =} \DecValTok{1}\OperatorTok{:}\DecValTok{3}\NormalTok{, }\DataTypeTok{b =} \DecValTok{4}\OperatorTok{:}\DecValTok{6}\NormalTok{, }\DataTypeTok{c =} \DecValTok{7}\OperatorTok{:}\DecValTok{9}\NormalTok{)}
\NormalTok{(aa <-}\StringTok{ }\NormalTok{df[df}\OperatorTok{$}\NormalTok{a }\OperatorTok{==}\StringTok{ }\DecValTok{2}\NormalTok{,])}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{(df[, }\KeywordTok{c}\NormalTok{(}\StringTok{"b"}\NormalTok{, }\StringTok{"c"}\NormalTok{)])}
\end{Highlighting}
\end{Shaded}
...
{\centering \includegraphics[width=0.7\linewidth]{figuras/dia1} 
...
\begin{figure}
\includegraphics[width=4.86in]{figuras/gway} \caption{Driving route between zones 'Luz' and 'Bom Retiro' in São Paulo}\label{fig:unnamed-chunk-39}
\end{figure}
...
\caption{\label{tab:unnamed-chunk-42}speeds for scheme = T}
\centering
\begin{tabular}[t]{cc}
\toprule
Period & Speed\\
...
\caption{\label{tab:unnamed-chunk-47}Most used combinations for emission factors for LDV}
\centering
\begin{tabular}[t]{ll}
\toprule
Argument & Value\\
...
\includegraphics{veinbook_files/figure-latex/unnamed-chunk-49-1.pdf}
...
\begin{verbatim}
## function(V) x[j,i]
## <bytecode: 0x55fdd6165538>
## <environment: 0x55fdedec3038>
\end{verbatim}
...
\item
  \texttt{emis\_wear} for wear of brakes, tires, and roads.
\end{itemize}
...
The highest emissions by the hour are of Light Trucks of 17 years of use
at hour 11 on local time. However, it is possible to get more
interesting insights into the data. The data can be aggregated by age,
hour or plot all together as shown in Fig. \ref{fig:noxlt3}. The highest
emissions occurred between 17 and 27 years of use. This figure also
shows the effect of emissions standards introduced in different years.
...
create a function that reads and merge the emissions that are in each
directory, returning a data-frame os a spatial feature data-frame. The
function is \texttt{emis\_merge}. Fig. \ref{fig:demispost},
\texttt{emis\_merge} comes after \texttt{emis\_post} or
\texttt{speciate}. The arguments of \texttt{emis\_merge} are:
...
\begin{verbatim}
## files at /tmp/RtmpSNspU0/YourCity
\end{verbatim}

\begin{verbatim}
## Directories:
##  [1] "/tmp/RtmpSNspU0/YourCity"             
##  [2] "/tmp/RtmpSNspU0/YourCity/ef"          
##  [3] "/tmp/RtmpSNspU0/YourCity/emi"         
##  [4] "/tmp/RtmpSNspU0/YourCity/emi/BUS_01"  
##  [5] "/tmp/RtmpSNspU0/YourCity/emi/HGV_01"  
##  [6] "/tmp/RtmpSNspU0/YourCity/emi/LCV_01"  
##  [7] "/tmp/RtmpSNspU0/YourCity/emi/MC_01"   
##  [8] "/tmp/RtmpSNspU0/YourCity/emi/PC_01"   
##  [9] "/tmp/RtmpSNspU0/YourCity/est"         
## [10] "/tmp/RtmpSNspU0/YourCity/images"      
## [11] "/tmp/RtmpSNspU0/YourCity/network"     
## [12] "/tmp/RtmpSNspU0/YourCity/post"        
## [13] "/tmp/RtmpSNspU0/YourCity/post/df"     
## [14] "/tmp/RtmpSNspU0/YourCity/post/grids"  
## [15] "/tmp/RtmpSNspU0/YourCity/post/streets"
## [16] "/tmp/RtmpSNspU0/YourCity/profiles"    
## [17] "/tmp/RtmpSNspU0/YourCity/veh"
\end{verbatim}

In my case, the files are in directory /tmp/RtmpSNspU0/YourCity. Now, if
...
is the network, where the user reads and prepares the traffic
information. Then the second section runs a script which runs the age
functions for the default vehicular composition of the
\texttt{inventory} function. The third section estimates the emissions
by running each input.R file inside the directory \emph{est} The fourth
section, creates a grid and run the script post.R is shown below:
...
all the files with names CO and the default argument \texttt{what} is
``STREETS.rds'', for instance, PC\_GASOLINE\_CO\_STREETS.rds. Hence,
this function reads all the files under that condition. The other
argument is \texttt{streets} with default value of \texttt{TRUE},
meaning that the function now knows that the resulting object must be a
spatial network, and the \texttt{net} argument is already calling the
net object part of the VEIN data, the function merges all the objects
summing the emissions of all vehicle street by street. The resulting
object is a spatial ``LINESTRING'' class of \texttt{sf}. This function
assumes that emissions files are data.frames and no tests have been made
to see if this function would read work \texttt{sf} objects.

On the other hand, the line 12 also runs all the emissions files whose
names include the word CO, but also including the words ``DF.rds'',
which are the files from \texttt{emis\_post} with argument \texttt{by}
equals to ``veh''. In this case, it reads and merges the data-frames and
prints the sum emissions by pollutant.

Sourcing the file main.R runs each line of this script, estimating
emissions and producing some plots.
...
Once we have processed our emissions array with \texttt{emis\_post} and
we have spatial emissions, we can then create grids and allocate our
emissions into the grids. Let's check the function \texttt{make\_grid}
first:
...
converted to \texttt{sf}. Then, as the coordinate reference system of
the net is WGS84 with epsg 4326, we are transforming to UTM to create a
grid with a grid spacing of 500 m. The class of the grid object,
\texttt{g} is ``sf data.frame''.
...
each grid cell. The process is performed by the intersection between
geometries and the grid. It means that requires ``sr'' according to with
...
function returns an object class ``sf''.
...
could not believe it. However, there also some I could accelerate this
process importing some \texttt{data.table} Aggregation functions. Now
\texttt{emis\_grid} can aggregate large spatial extensions. I haven't
tried yet at continental scale. Let's see the arguments:
...
\caption{\label{tab:unnamed-chunk-86}Black Carbon and Organic Matter [g/168h]}
\centering
...
It is interesting to see that break emissions are higher than tire wear
emissions, and the smaller the diameter, higher the difference, as
shown:

\begin{table}

\caption{\label{tab:unnamed-chunk-88}Ratio between break and tire wear emissions}
\centering
\begin{tabular}[t]{l|r}
\hline
  & x\\
...
The procedure can be used with \texttt{emis\_post} with
...
of vehicle, fuel, and euro standard. The following example will consist
...
\begin{figure}
\includegraphics[width=6.81in]{figuras/wrf} \caption{http://www.lapat.iag.usp.br/aerossol/wrf9/index.php}\label{fig:unnamed-chunk-92}
\end{figure}

Some of these papers show measurements in tunnels and fuel. The
...
speciation is more representative of Brazilian gasoline, which means?
...
Moreover, the user who wants to apply it outside Brazil should be
ensured, that the fleet and fuel are compatible. As that will be hard to
...
\begin{figure}
\includegraphics[width=12.36in]{figuras/iag1} \caption{Speciation for NMHC by type of fuel and process mol/100g (Ibarra-Espinosa, S., 2017)}\label{fig:unnamed-chunk-93}
\end{figure}
...
\begin{verbatim}
## Units: [mol/h]
## [1] 0.9258232 0.9276505 0.9218701
## Units: [mol/h]
## [1] 1.425646 1.428460 1.419559
## Units: [mol/h]
## [1] 0.5196056 0.5206312 0.5173869
## Units: [mol/h]
## [1] 0.2507228 0.2512177 0.2496523
## Units: [mol/h]
## [1] 1.902322 1.906076 1.894199
## Units: [mol/h]
## [1] 0.7975280 0.7991021 0.7941227
## Units: [mol/h]
## [1] 0.5287335 0.5297771 0.5264759
## Units: [mol/h]
## [1] 0.02536203 0.02541208 0.02525373
## Units: [mol/h]
## [1] 0.4602699 0.4611784 0.4583046
## Units: [mol/h]
## [1] 0.8768888 0.8786195 0.8731446
## Units: [mol/h]
## [1] 1.231729 1.234160 1.226470
## Units: [mol/h]
## [1] 0.3452714 0.3459529 0.3437971
## Units: [mol/h]
## [1] 0.4503067 0.4511955 0.4483840
## Units: [mol/h]
## [1] 0 0 0
## Units: [mol/h]
## [1] 0 0 0
...
\caption{\label{tab:unnamed-chunk-95}Speciation of PM2.5}
\centering
...
\begin{verbatim}
## Sum of street emissions 601875.82
\end{verbatim}
...
\begin{verbatim}
## Sum of gridded emissions 601875.82
\end{verbatim}
...
## 1 0.001440819 [g/m^2/s] 0.01165754 0.0004621849 0.001970367 0.01871194
## 2 0.001432126 [g/m^2/s] 0.01158720 0.0004593962 0.001958479 0.01859904
## 3 0.001467979 [g/m^2/s] 0.01187728 0.0004708971 0.002007509 0.01906466
## 4 0.002543292 [g/m^2/s] 0.02057755 0.0008158353 0.003478035 0.03302977
## 5 0.005775954 [g/m^2/s] 0.04673272 0.0018528061 0.007898805 0.07501239
## 6 0.000000000 [g/m^2/s] 0.00000000 0.0000000000 0.000000000 0.00000000
##     e_pm2.5j      e_orgi     e_orgj      e_eci       e_ecj        h2o
## 1 0.05613581 0.005688429 0.02425067 0.01047869 0.004490865 0.05183207
## 2 0.05579711 0.005654107 0.02410435 0.01041546 0.004463769 0.05151933
## 3 0.05719398 0.005795657 0.02470780 0.01067621 0.004575519 0.05280911
## 4 0.09908930 0.010041049 0.04280658 0.01849667 0.007927144 0.09149246
## 5 0.22503717 0.022803767 0.09721606 0.04200694 0.018002974 0.20778432
...
The emission file is generated with traffic simulation from the Traffic
Engineering Company (CET) for 2012, which consists in traffic flow for
morning rush hour of Light-Duty Vehicles (LDV). The emission factors are
the data \texttt{fe2015} and the wrfinput comes from the
...
in the object net, which is in km but has no units. We must add the
right units to use \texttt{vein}
...
The wrfchemi input file was created on a temporary directory with the
...
Then, we check our resulting NetCDF file plotting the emissions we just
put.
