...
VEIN improts functions from other R packages. HEnce, these packages are necessary::

**sf** is a package that provides simple features access for R [@sf]. It depends on the library
GEOS (http://trac.osgeo.org/geos), 'Geospatial' Data Abstraction Library ('GDAL')
(http://www.gdal.org/) and PROJ (http://proj4.org/).
**data.table** is a package that provides simple features access for R [@sf]. It can be
installed.
- **sp**. This is a package that provides classes and methods for spatial data [@sp] 
considering points, lines, polygons and grids. few function of sp are imported, most of them 
comes from sf.
...
In order to install **sf** package it is necessary to install the 
